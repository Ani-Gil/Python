...
def dispMessage(text):
    ...
    textps.center = (int(pad_width / 2), int(pad_height / 2))
    gamepad.blit(text, textpos)
    pygame.display.update()
    sleep(2)
    runGame()

# 전투기가 적과 충돌했을 떄 메시지 출력
def crash():
    global gamepad
    dispMessage('Crashed!')
    ...
    while not ongame:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # 왼쪽 컨트롤키를 누르면 총알이 발사. 총알은 한 번에 2발만 발사
        ...
        if y < enemy_y + enemy_height:
            if (enemy_x > x and enemy_x < x + fight_width) or \
                    (enemy_x + enemy_width > x and enemy_x + enemy_width < x +
fight_width):
                    crash()
...
def initGame():
    ...
    fighter = pygame.image.load('fighet.png')
